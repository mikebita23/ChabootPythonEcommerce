...
def gmail_request(shop,req_type,ressource,endpoints="",post_data={}):
    ...
    while tries < 2:
        try:
            ...
            if req_type == "get":
                r = requests.get(cont,headers={"Authorization":"OAuth {}".format(access_tokens[shop])}, timeout=50)
                print(r.json())
            elif req_type == "post":
            ...
            if r.status_code > 300:
                ...
                r2 = requests.post("https://www.googleapis.com/oauth2/v4/token",data=data, timeout=500)
                print(r2.json())
...
def detect_question(text,subject):
    ...
    if l == 0:
        ...
        #pattern1 = r"Commande #\d|1\d|20 confirmée"
        #pattern2 = r"Une commande #\d|1\d|20 est en transit"
        pattern1 = r"Commande #\([01]?\d) | 20 confirmée"
        pattern2 = r"Une commande #\([01]?\d) | 20 est en transit"
