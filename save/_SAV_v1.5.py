# -*- coding: utf-8 -*-
path = "C:/Users/78kid/OneDrive/Bureau/programmes/_SAV/"

#######################################################
################ SAV V1.5 by W.SELLIER ################
#######################################################
...
def gmail_request(shop, req_type, ressource, endpoints="", post_data={}):
    ...
    while tries < 20:
        try:
            cont = "https://www.googleapis.com/gmail/v1/users/{}/{}?{}key={}".format(user_id[shop], ressource,
                                                                                     endpoints, api_key[shop])
            if req_type == "get":
                r = requests.get(cont, headers={"Authorization": "OAuth {}".format(access_tokens[shop])})
            elif req_type == "post":
                r = requests.post(cont, headers={"Authorization": "OAuth {}".format(access_tokens[shop])},
                                  json=post_data)
            if r.status_code > 300:
                data = {"client_id": client_id[shop], "client_secret": client_secret[shop],
                        "refresh_token": refresh_token[shop], "grant_type": "refresh_token"}
...
def get_tracking_by_scraping(tracking_num):
    months = {"Jan": 1, "Fév": 2, "Mar": 3, "Avr": 4, "May": 5, "Jun": 6, "Jul": 7, "Aug": 8, "Sep": 9, "Oct": 10,
              "Nov": 11, "Déc": 12}
...
def fb_send(message, thread_id):
    ...
    while tries < 20:
        try:
            ...
            for line in lines:
                message_id = fb_client.send(models.Message(text=line), thread_id=thread_id,
...
def build_answer(shop, order_info, message, questions, history):
    ...
    for j in range(len(history)):
        if (message["email"] in history[j]):
            if history[j][2] == "no":  # Question and then customer_id
                ...
                response = "/ALERT Je n'ai pas d'informations dur ce client"
                answer_id = "s9"

            answer_ids = list(eval(history[j][4]))
            if answer_id not in answer_ids:
                answer_ids.append(answer_id)
            else:
                return "/ALERT Ce client m'a déja posé la même question", "/ALERT"
    ...
    answer_ids = []
    answer_ids.append(answer_id)
    file = open("{}/{}/history.csv".format(path, shop), "a")
    file.write(
        "\n{},{},{},{},{}".format(message["email"], str(datetime.now())[:-7], info, str(questions).replace(",", "$"),
                                  str(answer_ids).replace("\r", "")))
    file.close()
    return response, answer_id
...
while True:
    ...
    for shop in shops:
        # GMAIL:
        r = gmail_request(shop, "get", "messages", endpoints="maxResults=5&q=label:inbox&")
            ...
            if message_id not in lst_message_ids:
                ...
                for kjkj in "a":
                    ...
                    if message["source_language"] == "ERROR":
                        message_text = "Je n'ai pas pu détecter la langue du client"
                        mime_msg = create_mime_msg(sav_mail[shop], message_text, message)
                        gmail_request(shop, "post", "drafts", endpoints="", post_data={"message": mime_msg})
                        today_nt += 1
                        print("history saved ! waiting for another message\n")
...
"""
        ...
        if unreads != []:
            for thread_id in unreads:
                ...
                if text != "" and sender != "William" and text[0] != "$":
                    clients = build_clients_dic()
                    for client in clients.keys():
                    ...
                    if cont == "":
                        ...
                        ai = apiai.ApiAI("5748c522a6614cd38896b9b704ea6462")
                        session_id = lst2[0]
                        contexts2 = eval(lst2[5].replace("$",","))
                        rep_text, contexts = apiai_request(ai,text,session_id,contexts2)
                        answer = rep_text
                        shop = lst2[2]
                        ...
                        if "/END" not in rep_text:
                            ...
                            az = "\n".join(lst[1:])
                            file = open("{}/clients/{}/messenger_assistant.csv".format(path,owner),"w")
                            file.write("{},{},{},{},{},{}\n{}".format(session_id,action,shop,str(message).replace(",","$"),str(order_info).replace(",","$"),str(contexts).replace(",","$"),az))
                            file.close()
