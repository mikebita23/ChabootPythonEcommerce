...
def shopify_request(shop,req_type,ressource,endpoints="",put_data={}):
    ...
    if req_type == "get":
        r = requests.get("{}{}{}".format(shopify_api_url[shop],ressource,endpoints), timeout=15)
    elif req_type == "post":
...
def build_messages_dic(shops):
    ...
    for n in range(1,3):
        for shop in shops:
            file = codecs.open("{}/messages_dic_{}.csv".format(shop,n),encoding="utf-8")
            cont = file.read()
            file.close()
            lst = cont.split("\n")[2:]
            lst2, lst3, lst4 = [], [], []
...
def build_history(shop):
    history = []
    file = open("{}/history.csv".format(shop),"r")
    cont = file.read()
    file.close()
    lst = cont.split("\n")
...
def update_order_ids(shop,first_order_ids):
    file = open("{}/order_ids.csv".format(shop),"r")
    init = file.read()
    file.close()
    new = ""
    if len(init) > 5:
    ...
    file2 = open("{}/order_ids.csv".format(shop),"w")
...
def build_answer(shop,order_info,message,questions,history,is_buffalo):
    ...
    for j in range(len(history)):
        if (message["email"] in history[j]):
            ...
            cont = "\n".join(lst)
            file = open("{}/history.csv".format(shop),"w")
            file.write(cont)
            file.close()
    ...
    answer_ids = []
    answer_ids.append(answer_id)
    file = open("{}/history.csv".format(shop),"a")
...
def fb_new(shop,questions,order_info,message,answer_id,menace,score):
    if "modify_adress" in questions and order_info == {}:
        file = open("{}/messages_cache.csv".format(shop),"r")
        cont = file.read()
        file.close()
        dic = eval(cont.replace("$",","))
        dic[message["email"]] = str(message).replace(",","$")
        file = open("{}/messages_cache.csv".format(shop),"w")
        file.write(str(dic))
        file.close()
        ...
        if unreads != []:
            for thread_id in unreads:
                ...
                if text != "" and sender != "William" and text[0] != "$":
                    ...
                    file = open("clients/{}/messenger_assistant.csv".format(owner),"r")
                    cont = file.read()
                    file.close()
                    if cont == "":
